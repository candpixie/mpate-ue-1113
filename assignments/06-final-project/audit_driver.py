...
# Line: 想約他 私訊他 別無視我  (10 syllables)
H_GE_LEI = {
    'song':     '隔離',
    'artist':   'Jace Chan',
    'source':   'human',
    'tempo':    'mid',
    'lyrics':   ['想','約','他','私','訊','他','別','無','視','我'],
    'jyutping': ['soeng2','joek3','taa1','si1','seon3','taa1',
                 'bit6','mou4','si6','ngo5'],
    'tones':    [2, 3, 1, 1, 3, 1, 6, 4, 6, 5],
    'snippet_range': (25, 30),
    'syllable_times': [],
...
# ---- SUNO MID: 玻璃 chorus "留低好嗎 留低一晚" (8 syllables) ----
# (audio file is named jace_gelei-suno.mp3 but contains 玻璃 generation)
# Caveat: 嗎 (maa3) is a Mandarin-loaned sentence particle. Native
# Cantonese question marker would be 啊/咩. Including this as-is.
S_SUNO_MID = {
    'song':     '玻璃 (Suno)',
    'artist':   'Suno V5.5',
    'source':   'suno',
    ...
    'tones':    [4, 1, 2, 3, 4, 1, 1, 5],
    'snippet_range': (62, 68),   # 1:02-1:08
    'syllable_times': [],
...
# Snippet from verse: "你快樂過生活 我拼命去生存"  (12 syllables)
# (The other iconic line "我在高山 我在低谷" is the chorus of this same song.)
H_GOU_SAAN = {
    'song':     '高山低谷',
    'artist':   'Phil Lam (林奕匡)',
    'source':   'human',
...
# ---- SUNO BALLAD: 雨窗一封 pre-chorus
# "如果我肯早啲講 會唔會唔同結果" (14 syllables) ----
# Caveat: 會 has wui5/wui6 ambiguity. Committing to wui5 (T5/M) as the
# modern Cantonese auxiliary reading.
S_SUNO_BALLAD = {
    'song':     '雨窗一封 (Suno)',
    'artist':   'Suno V5.5',
    'source':   'suno',
    'tempo':    'ballad',
    'lyrics':   ['如','果','我','肯','早','啲','講',
                 '會','唔','會','唔','同','結','果'],
    'jyutping': ['jyu4','gwo2','ngo5','hang2','zou2','di1','gong2',
                 'wui5','m4','wui5','m4','tung4','git3','gwo2'],
    'tones':    [4, 2, 5, 2, 2, 1, 2, 5, 4, 5, 4, 4, 3, 2],
    'snippet_range': (71, 77),   # 1:11-1:17
    'syllable_times': [],
...
# ---- SUNO UPTEMPO: track titled 旺角快車 in Suno, but the audio
# at 1:11-1:14 actually sings lyrics from a DIFFERENT prompt (雨窗一封):
# "喺原地 等一個唔會返嚟的你" (12 syllables).
# This is itself an AI failure mode: Suno hallucinated lyrics that
# weren't in the 旺角快車 prompt. We audit what's actually sung.
# Tone caveats: 的 (dik1) is Mandarin-loaned, 會 wui5/wui6 ambiguity.
S_SUNO_UPTEMPO = {
    'song':     '旺角快車 (Suno, mis-rendered)',
    'artist':   'Suno V5.5',
    'source':   'suno',
    'tempo':    'uptempo',
    'lyrics':   ['喺','原','地','等','一','個','唔',
                 '會','返','嚟','的','你'],
    'jyutping': ['hai2','jyun4','dei6','dang2','jat1','go3','m4',
                 'wui5','faan1','lai4','dik1','nei5'],
    'tones':    [2, 4, 6, 2, 1, 3, 4, 5, 1, 4, 1, 5],
    'snippet_range': (71, 74),   # 1:11-1:14
    'syllable_times': [],
...
def plot_audit(df, save_path='figures/human_vs_suno.png'):
    ...
    human = [df[(df['tempo']==t) & (df['source']=='human')]['match_rate'].mean()
             for t in tempos]
    suno  = [df[(df['tempo']==t) & (df['source']=='suno')]['match_rate'].mean()
             for t in tempos]
    human_labels = [df[(df['tempo']==t) & (df['source']=='human')]['song'].iloc[0]
                    if len(df[(df['tempo']==t) & (df['source']=='human')]) else ''
                    for t in tempos]
    suno_labels = [df[(df['tempo']==t) & (df['source']=='suno')]['song'].iloc[0]
                   if len(df[(df['tempo']==t) & (df['source']=='suno')]) else ''
                   for t in tempos]

    fig, ax = plt.subplots(figsize=(9, 5.5))
    bars_h = ax.bar(x - width/2, human, width, label='Human', color='#2E86AB')
    bars_s = ax.bar(x + width/2, suno,  width, label='Suno',  color='#E63946')
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title('Cantonese tone-tune match rate by tempo: Human vs Suno',
                 fontsize=13)
    ax.legend(loc='lower right', fontsize=10)
    ax.grid(axis='y', alpha=0.3)

    for i, (h, s, hl, sl) in enumerate(zip(human, suno, human_labels, suno_labels)):
        if not np.isnan(h):
