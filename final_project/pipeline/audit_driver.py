...
PIPELINE:
  1. Define dataset (4 human + 4 Suno chorus snippets, matched by tempo).
  2. Auto-segment syllable timings via Parselmouth voicing (or manual).
  3. Run F0 extraction + tone-tune audit per snippet.
  4. Aggregate human vs Suno match/violation rates by pool.
  5. Plot results for the video.
"""

# Normalize cwd to final_project/ so relative paths to audio/ and
# figures/ work regardless of where this script is invoked from
# (pipeline/ is a subfolder; audio/ and figures/ live one level up).
import os, pathlib
...
#
# **Method.** 4 human Cantopop chorus lines paired against 4 Suno-generated
# chorus lines, matched by tempo (mid / ballad / uptempo). Suno wrote its
# own lyrics from style prompts. All snippets are 10-14 syllables, drawn
# from the chorus of each track.
